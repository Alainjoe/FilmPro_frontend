...
document.addEventListener('DOMContentLoaded', () => {
    if (filmId) checkAuth();
    else showError('ID du film non spécifié.');
});
...
function displayFilmDetails(film) {
    ...
    if (film.acteurs) {
        const allActors  = film.acteurs.split(',').map(a => a.trim()).filter(Boolean);
        const mainActors = allActors.slice(0, 10).map(actor => `<div class="actor-item">${actor}</div>`);
    ...
    const posterContent = (film.imgPath && film.imgPath.includes('themoviedb.org'))
        ? `<img src="${film.imgPath}" alt="${film.title}" onerror="this.style.display='none';">`
        : `
...
async function louerFilm(filmId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/location/location`, { // ✅ corrigé
            method: 'POST',
